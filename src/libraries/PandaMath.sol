...
library PandaMath {
    ...
    uint256 internal constant FEE_SCALE = 10_000;
    ...
    function getDexPair(address pandaToken, address baseToken, address v2Factory, bytes32 initCodeHash)
        internal
        pure
        returns (address pair)
    {
        ...
        pair = address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(hex"ff", v2Factory, keccak256(abi.encodePacked(token0, token1)), initCodeHash)
                    )
                )
            )
        );
