...
contract IslandRouter is IIslandRouter {
    ...
    function addLiquiditySingleNative(
        ...
        require(maxStakingSlippageBPS <= 10_000, "staking slippage too high");
        ...
        (uint256 token0Balance, uint256 token1Balance) =
            _swapAndVerify(token0, token1, IERC20(address(wBera)), swapData);
        ...
        if (__isToken0WBera) {
            require(
                amount1 >= (token1Balance + amount1) * (10_000 - maxStakingSlippageBPS) / 10_000,
                "Staking Slippage: below min amounts"
            );
            ...
            require(
                amount0 >= (token0Balance + amount0) * (10_000 - maxStakingSlippageBPS) / 10_000,
                "Staking Slippage: below min amounts"
            );
            _deposit(island, amount0, amount1, mintAmount, receiver);
            if (token1Balance > 0) {
    ...
    function addLiquiditySingle(
        ...
        if (swapData.zeroForOne) {
            require(
                amount1 >= token1Balance * (10_000 - maxStakingSlippageBPS) / 10_000,
                "Staking Slippage: below min amounts"
            );
        } else {
            require(
                amount0 >= token0Balance * (10_000 - maxStakingSlippageBPS) / 10_000,
                "Staking Slippage: below min amounts"
            );
        }
    ...
    function _addLiquidity(
        ...
        require(
            amount0In >= amount0Min && amount1In >= amount1Min && _mintAmount >= amountSharesMin, "below min amounts"
        );
    ...
    function _deposit(IKodiakIsland island, uint256 amount0In, uint256 amount1In, uint256 _mintAmount, address receiver)
        internal
    ...
    function _swapAndVerify(IERC20 token0, IERC20 token1, IERC20 tokenIn, RouterSwapParams calldata swapData)
        internal
        returns (uint256 token0Balance, uint256 token1Balance)
    {
        ...
        if (address(token0) == address(tokenIn)) {
            require(token1Balance >= swapData.minAmountOut, "Swap: insufficient tokenOut");
        } else {
            require(token0Balance >= swapData.minAmountOut, "Swap: insufficient tokenOut");
        }
    }
}
