...
abstract contract KodiakIsland is IUniswapV3MintCallback, IUniswapV3SwapCallback, KodiakIslandStorage {
    ...
    event Rebalance(
        address indexed compounder, int24 lowerTick_, int24 upperTick_, uint128 liquidityBefore, uint128 liquidityAfter
    );
        ...
        returns (uint256 amount0, uint256 amount1, uint128 liquidityMinted)
    {
        ...
        liquidityMinted = LiquidityAmounts.getLiquidityForAmounts(
            sqrtRatioX96, lowerTick.getSqrtRatioAtTick(), upperTick.getSqrtRatioAtTick(), amount0, amount1
        );
        ...
        returns (uint256 amount0, uint256 amount1, uint128 liquidityBurned)
    {
        ...
        amount0 =
            burn0 + FullMath.mulDiv(token0.balanceOf(address(this)) - burn0 - managerBalance0, burnAmount, totalSupply);
        amount1 =
            burn1 + FullMath.mulDiv(token1.balanceOf(address(this)) - burn1 - managerBalance1, burnAmount, totalSupply);
        ...
        returns (uint256 amount0, uint256 amount1, uint256 mintAmount)
    {
        ...
        if (totalSupply > 0) {
            ...
            uint128 newLiquidity = LiquidityAmounts.getLiquidityForAmounts(
                sqrtRatioX96, lowerTick.getSqrtRatioAtTick(), upperTick.getSqrtRatioAtTick(), amount0Max, amount1Max
            );
            mintAmount = uint256(newLiquidity);
            (amount0, amount1) = LiquidityAmounts.getAmountsForLiquidity(
                sqrtRatioX96, lowerTick.getSqrtRatioAtTick(), upperTick.getSqrtRatioAtTick(), newLiquidity
            );
        }
    }
        ...
        returns (uint256 amount0Current, uint256 amount1Current)
    {
        ...
        (amount0Current, amount1Current) = LiquidityAmounts.getAmountsForLiquidity(
            sqrtRatioX96, lowerTick.getSqrtRatioAtTick(), upperTick.getSqrtRatioAtTick(), liquidity
        );
    ...
    function _withdraw(int24 lowerTick_, int24 upperTick_, uint128 liquidity)
        internal
    ...
    function _deposit(
        ...
        int256 swapAmount = SafeCast.toInt256(((zeroForOne ? amount0 : amount1) * swapAmountBPS) / 10_000);
        if (swapAmount > 0) {
    ...
    function _swap(uint256 amount0, uint256 amount1, int256 swapAmount, uint160 swapThresholdPrice, bool zeroForOne)
        private
    ...
    function _mintMaxLiquidity(int24 lowerTick_, int24 upperTick_, uint256 maxAmount0, uint256 maxAmount1)
        internal
        returns (uint256 amount0, uint256 amount1)
    {
        (uint160 sqrtRatioX96,,,,,,) = pool.slot0();
        uint128 liquidityMinted = LiquidityAmounts.getLiquidityForAmounts(
            sqrtRatioX96, lowerTick_.getSqrtRatioAtTick(), upperTick_.getSqrtRatioAtTick(), maxAmount0, maxAmount1
        );
        if (liquidityMinted > 0) {
            (uint256 deposited0, uint256 deposited1) =
                pool.mint(address(this), lowerTick_, upperTick_, liquidityMinted, "");
            amount0 = maxAmount0 - deposited0;
            amount1 = maxAmount1 - deposited1;
        }
    }

    function _computeMintAmounts(uint256 totalSupply, uint256 amount0Max, uint256 amount1Max)
        private
        view
        returns (uint256 amount0, uint256 amount1, uint256 mintAmount)
    {
    ...
    function _computeFeesEarned(bool isZero, uint256 feeGrowthInsideLast, int24 tick, uint128 liquidity)
        private
        view
        returns (uint256 fee)
    {
    ...
    function _applyFees(uint256 _fee0, uint256 _fee1) internal {
        ...
        if (managerFeeBPS > 0) {
            managerBalance0 += _fee0 * _managerFeeBPS / 10_000;
            managerBalance1 += _fee1 * _managerFeeBPS / 10_000;
    ...
    function _subtractManagerFee(uint256 _fee0, uint256 _fee1, uint16 _managerFeeBPS)
        internal
        pure
        returns (uint256 fee0, uint256 fee1)
    {
        fee0 = _fee0 - _fee0 * _managerFeeBPS / 10_000;
        fee1 = _fee1 - _fee1 * _managerFeeBPS / 10_000;
    }
}
