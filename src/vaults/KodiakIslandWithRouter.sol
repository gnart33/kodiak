...
contract KodiakIslandWithRouter is KodiakIsland {
    ...
    function executiveRebalanceWithRouter(int24 newLowerTick, int24 newUpperTick, SwapData calldata swapData)
        external
        whenNotPaused
        onlyManager
    {
        ...
        {
            uint256 worstOut = worstAmountOut(
                swapData.amountIn, compounderSlippageBPS, getAvgPrice(compounderSlippageInterval), swapData.zeroForOne
            );
    ...
    function worstAmountOut(uint256 amountIn, uint16 slippageBPS, uint160 avgSqrtPriceX96, bool zeroForOne)
        public
        pure
        returns (uint256)
    {
        ...
        uint256 slippage = uint256(avgSqrtPriceX96) * slippageBPS / 10_000;
    ...
    function _swapWithRouter(uint256 amount0, uint256 amount1, SwapData calldata swapData)
        internal
