...
contract XKodiakToken is Ownable, ReentrancyGuard, ERC20("Kodiak pre-TGE rewards", "xKDK"), IXKdkToken {
    ...
    event UpdateRedeemRewardsAddress(
        address indexed userAddress, uint256 redeemIndex, address previousRewardsAddress, address newRewardsAddress
    );
    ...
    function getXKodiakBalance(address userAddress)
        external
        view
    ...
    function isWhitelister(address account) public view override returns (bool) {
    ...
    function updateRedeemSettings(
        ...
        require(
            maxRedeemRatio_ <= MAX_FIXED_RATIO && redeemRewardsAdjustment_ <= MAX_FIXED_RATIO,
        ...
        emit UpdateRedeemSettings(
            minRedeemRatio_, maxRedeemRatio_, minRedeemDuration_, maxRedeemDuration_, redeemRewardsAdjustment_
        );
    }
    ...
    function redeem(uint256 xKdkAmount, uint256 duration) external kdkActive nonReentrant {
        ...
        if (duration > 0) {
            ...
            userRedeems[msg.sender].push(
                RedeemInfo(
                    kdkAmount, xKdkAmount, _currentBlockTimestamp().add(duration), rewardsAddress, rewardsAllocation
                )
            );
        } else {
    ...
    function updateRedeemRewardsAddress(uint256 redeemIndex)
        external
        kdkActive
        nonReentrant
        validateRedeem(msg.sender, redeemIndex)
    {
        ...
        if (rewardsAddress != _redeem.rewardsAddress && address(rewardsAddress) != address(0)) {
            ...
            emit UpdateRedeemRewardsAddress(
                msg.sender, redeemIndex, address(_redeem.rewardsAddress), address(rewardsAddress)
            );
    ...
    /// @notice args specific to usage contract must be passed into "usageData"
    function deallocate(address usageAddress, uint256 amount, bytes calldata usageData)
        external
        kdkActive
        nonReentrant
    {
    ...
    function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
        ...
        uint256 deallocationFeeAmount = amount.mul(usagesDeallocationFee[usageAddress]).div(10_000);
