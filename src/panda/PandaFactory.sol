...
contract PandaFactory is Ownable, IPandaFactory, ReentrancyGuard {
    ...
    uint256 public constant override MIN_TOKENSINPOOL_SHARE = 5000; //50%
    uint256 public constant override MAX_TOKENSINPOOL_SHARE = 9000; //90%
    uint256 public constant override MIN_SQRTP_MULTIPLE = 11_000; //1.1x in sqrtP minimum distance
    uint256 public constant override MAX_SQRTP_MULTIPLE = 10 * 10_000; //10x in sqrtP = 100x in P = ~90% tokensInPool

    //Panda Token Defaults
    uint256 public constant override TOKEN_SUPPLY = 1_000_000_000 * 1e18;
    uint16 public constant override DEPLOYER_MAX_BPS = 5000; // 50%
    ...
    ) external override nonReentrant returns (address pandaToken) {
    ...
    function deployPandaTokenWithBera(
        ...
        string calldata symbol,
        uint16 deployerSupplyBps
    ) external payable override nonReentrant returns (address pandaToken) {
    ...
    function deployPandaPool(
        ...
        //Initialize
        pandaPool.initializePool(pandaToken, pp, totalTokens, msg.sender, data);
    ...
    function _deployPandaToken(
        ...
        //Initialize
        {
            ...
            pandaToken.initializePool(address(pandaToken), pp, TOKEN_SUPPLY, msg.sender, data);
        }
    ...
    function _buyTokens(address pool, address baseToken, uint16 deployerSupplyBps) internal {
        ...
        try IPandaPool(pool).getAmountInBuy(amountOut) returns (uint256 amountIn, uint256, uint256) {
            ...
            IPandaPool(pool).buyTokens(amountIn, amountOut * 9900 / 10_000, msg.sender);
        } catch {
            return;
        }
    }
    ...
    function _buyTokensWithBera(address pool, uint16 deployerSupplyBps) internal returns (uint256) {
        ...
        try IPandaPool(pool).getAmountInBuy(amountOut) returns (uint256 amountIn, uint256, uint256) {
            ...
            IPandaPool(pool).buyTokensWithBera{ value: amountIn }(amountOut * 9900 / 10_000, msg.sender);
            return amountIn;
        } catch {
            return 0;
        }
    ...
    function _checkDeploymentInputs(PandaPoolParams calldata pp, uint256 totalTokens, IPandaPool panda) internal view {
        ...
        require(
            tokensInPoolShare >= MIN_TOKENSINPOOL_SHARE && tokensInPoolShare <= MAX_TOKENSINPOOL_SHARE,
    ...
    function predictPoolAddress(address implementation, address deployer) external view override returns (address) {
    ...
    function setPandaPoolFees(uint16 _buyFee, uint16 _sellFee, uint16 _graduationFee, uint16 _deployerFeeShare)
        external
        override
        onlyOwner
    {
