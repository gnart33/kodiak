...
contract KodiakFarm is Ownable, ReentrancyGuard, Multicallable {
    ...
    modifier onlyTknMgrs(address reward_token_address) {
        require(
            msg.sender == owner() || isTokenManagerFor(msg.sender, reward_token_address), "Farm: Not owner or tkn mgr"
        );
        _;
    }
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            if (i > 0) {
                require(
                    rewardTokenAddrToIdx[_rewardTokens[i]] == 0 && _rewardTokens[i] != rewardTokens[0],
                    "Token already added"
                );
            }
        ...
        rewardsDuration = 30 * 86_400; // 30 * 86400  (30 days)
        lock_time_min = 0;
        lock_time_for_max_multiplier = 1 * 30 * 86_400; // 30 days
    ...
    function _stakeLocked(address user, uint256 liquidity, uint256 secs, uint256 start_timestamp)
        internal
        updateRewardAndBalance(user, true)
    {
    ...
    function _retroCatchUp() internal {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            bool haveTokensToRenew = rewardRates[i].mul(rewardsDuration).mul(num_periods_elapsed + 1)
    ...
    function setRewardsDuration(uint256 _rewardsDuration) external onlyOwner {
        require(_rewardsDuration >= 86_400, "Rewards duration too short");
    ...
    function setRewardRate(address _rewardToken, uint256 _rewardRate, bool sync_too)
        external
        onlyTknMgrs(_rewardToken)
    {
